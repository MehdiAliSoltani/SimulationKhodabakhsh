...
/**
 * PowerDatacenter class is a CloudResource whose hostList are virtualized. It
 * deals with processing of VM queries (i.e., handling of VMs) instead of
 * processing Cloudlet-related queries. So, even though an AllocPolicy will be
 * instantiated (in the init() method of the superclass, it will not be used, as
 * processing of cloudlets are handled by the CloudletScheduler and processing
 * of VirtualMachines are handled by the VmAllocationPolicy.
 *
...
public class FederatedDatacenter extends SimEntity {
    ...
    /**
     * this data center belong to which cloud in federation
     */
    private int cloudId;
//    private static final int COORDINATOR_CALL = 46789;
//    private static final int SHRINK = 12345;
    /**
     * The characteristics.
     */
...
//    protected CloudCoordinator coordinator;
    ...
    private final double CPU_HIGH_THRESHOLD = 0.95; //percent
    private final double RAM_HIGH_THRESHOLD = 0.95; //percent
    private final double OVER_AVERAGE = 0.05;
    private final int HOURE = 3600;
     ...
     * @post $none
     */
//	public FederatedDatacenter(int CloudId,
//                                    String name,
...
//		coordinator.setDatacenter(this);
    ...
    /**
     * this method calculate how many instances can places on the data center
     * calculate capacity of data center
     *
     * @return
     *//*
    public Map<Integer, int[]> getFreeResource() {
        final int CXL = 4, CM = 3, MXL = 2, ML = 1, MS = 0;

        int numberofresource = 2; //cpu and ram
        Map<Integer, int[]> map = new HashMap<Integer, int[]>();
        int[][] instanceSla = new int[numberofresource][Constants.VM_TYPES];
        instanceSla[0] = Constants.VM_MIPS;
        instanceSla[1] = Constants.VM_RAM;
        for (Host host : getHostList()) {
            int[] numofinstanse = new int[Constants.VM_TYPES];
            double mips = host.getAvailableMips();
            double ram = host.getRamProvisioner().getAvailableRam();

            while (true) {
                boolean flag = false;
                if (mips >= instanceSla[0][ML] && ram >= instanceSla[1][ML]) {
                    numofinstanse[ML] += 1;
                    mips = mips - instanceSla[0][ML];
                    ram = ram - instanceSla[0][ML];
                    flag = true;
                }
                if (mips >= instanceSla[0][MXL] && ram >= instanceSla[1][MXL]) {
                    numofinstanse[MXL] += 1;
                    mips = mips - instanceSla[0][MXL];
                    ram = ram - instanceSla[0][MXL];
                    flag = true;
                }

                if (mips >= instanceSla[0][CXL] && ram >= instanceSla[1][CXL]) {
                    numofinstanse[CXL] += 1;
                    mips = mips - instanceSla[0][CXL];
                    ram = ram - instanceSla[0][CXL];
                    flag = true;
                }

                if (mips >= instanceSla[0][CM] && ram >= instanceSla[1][CM]) {
                    numofinstanse[CM] += 1;
                    mips = mips - instanceSla[0][CM];
                    ram = ram - instanceSla[0][CM];
                    flag = true;
                }

                if (mips >= instanceSla[0][MS] && ram >= instanceSla[1][MS]) {
                    numofinstanse[MS] += 1;
                    mips = mips - instanceSla[0][MS];
                    ram = ram - instanceSla[0][MS];
                    flag = true;
                }
                if (!flag) {
                    break;
            ...
            for (int i = 0; i < Constants.VM_TYPES; i++) {
                if (numofinstanse[i] != 0) {
                    map.put(host.getId(), numofinstanse);
                    break;
                }
            }
        }
...
*/
    ...
    private void processShrink(SimEvent ev) {
        ev.getData();
    }
     ...
     * This method is called by {@link #body()} to register other type to GIS
     * entity. In doing so, you need to create a new child class extending from
    ...
    public void processEvent(SimEvent ev) {
        ...
        switch (ev.getTag()) {
            ...
            case CloudSimTags.VM_DATACENTER_EVENT:
                ...
                break;
//            case Constants.PROCESS_CREATEVM_In_HOST:
...
//        SdVm vm = (SdVm) ev.getData();
        ...
        SdHost sdhost = doShrinking(currenttime, vm);
        if (sdhost == null) {
            result = false;
        } else {
            result = getVmAllocationPolicy().allocateHostForVm(vm, sdhost);
        }
    ...
    protected void processVmDestroy(SimEvent ev, boolean ack) {
        Vm vm = (Vm) ev.getData();
//        SdHost h =(SdHost) vm.getHost();
//        System.out.println("vm.getHost().getRamProvisioner().getRam();"+h.getRamProvisioner().getAvailableRam());
...
//        SdVm sdvm = (SdVm) vm;
        ...
        int vmId;
        String Id = Integer.toString(vm.getId());// = (String)vmId;
        Id = Id.substring(1, Id.length());
        vmId = Integer.parseInt(Id);
        /*
        try {
            Resolver resolver = new Resolver();
            resolver.setFinishTime(cloudId, vmId, CloudSim.clock());
            resolver.setStatus(cloudId, vmId, Constants.VM_STATUS_TERMINATED, (int) CloudSim.clock(), null);
        } catch (Exception e) {
            int ss = sdvm.getCloudId();
            int cc = sdvm.getDestinationCloudId();
            int s = CloudTable.getCloudTable(cloudId).get(vmId).getSourcecloudId();
            int d = CloudTable.getCloudTable(cloudId).get(vmId).getOutsourcedcloudId();
            System.out.println("cloudId" + cloudId + " reuestId" + vmId + " s " + s + "  d " + d);
            System.out.println("vmId " + vmId);
        }*/
//        
//        Log.printLine(CloudSim.clock() + " : Vm# " + vmId + " from cloud #" + sdvm.getCloudId()
//                + " terminated in cloud #" + sdvm.getDestinationCloudId() + " successfully.");
//        CloudTable.getCloudTable(sdvm.getCloudId()).get(vmId).setStatus(Constants.VM_STATUS_TERMINATED);
        getVmList().remove(vm);
    }
